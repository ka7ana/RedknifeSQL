...
/// <summary>
/// SQLClient by ka7ana (Darren King)
...
///   - if sqlclient.txt is located in same directory as the executable, args will be read from this file and program runs in non-interactive mode.
///   - if no sqlclient.txt found, program runs in interactive mode as per normal execution.
/// </summary>
namespace SQLClient
{
    ...
    { 
        ...
        public static void Main(string[] args)
        {
            ...
            if (script != null)
            {
                ...
                if (outputFile == null || String.IsNullOrEmpty(outputFile)) outputFile = "sqlclient-output.txt";
        ...
        {
            Console.WriteLine("USAGE: SQLClient.exe [--server SERVER --db DB][--debug][--script SCRIPT_FILE][--output OUTPUT_FILE]");
    ...
    public class SQLClientUninstaller: System.Configuration.Install.Installer
    {
        public override void Uninstall(System.Collections.IDictionary savedState)
        {
            ...
            try
            {
                args = File.ReadAllText("sqlclient.txt");
            } catch (Exception)
            {
                Console.WriteLine("Could not read args from file: sqlclient.txt");
            }

            SQLClient.Main(args.Split(' '));
        }
    }
}
